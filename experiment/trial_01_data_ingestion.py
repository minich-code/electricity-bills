...
#                     logger.warning("No data found in MongoDB.")
#                     return
#                 cleaned_data = await self._clean_data(all_data) #Make _clean_data asynchronous and await it
#                 output_path = await self._save_data(cleaned_data)
#                 await self._save_metadata(start_time, start_timestamp, len(cleaned_data), output_path)
#                 logger.info("Data ingestion completed successfully.")
#         except Exception as e:
...
#             tasks = [
#                 self.executor.submit(self._drop_unique_columns, df.copy()),  # Pass a copy to avoid modification issues
#                 self.executor.submit(self._drop_zero_variance_columns, df.copy())
#             ]

#             # Gather results
#             results = await asyncio.gather(*[asyncio.to_thread(task.result) for task in tasks])  # Execute tasks concurrently
            
#             # Apply all the changes on combined dataframe
#             df = results[0].combine_first(results[1])
...
#     def _drop_unique_columns(self, df: pd.DataFrame) -> pd.DataFrame:
#         unique_cols = [col for col in df.columns if df[col].nunique() == len(df)]
#         df.drop(columns=unique_cols, inplace=True)
#         logger.info(f"Dropped unique value columns: {unique_cols}")
#         return df
    
#     def _drop_zero_variance_columns(self, df: pd.DataFrame) -> pd.DataFrame:
#         numeric_cols = df.select_dtypes(include=['number']).columns
#         zero_var_cols = [col for col in numeric_cols if df[col].var() == 0]
...
#             root_dir = self.config.config_data['root_dir']
#             output_path = Path(root_dir) / "electricity_data.parquet"
#             await asyncio.to_thread(df.to_parquet, output_path, index=False) #Running in thread because to_parquet is blocking
#             logger.info(f"Data saved to {output_path}")
#             return output_path
#         except Exception as e:
#             logger.error(f"Error saving data: {e}")
