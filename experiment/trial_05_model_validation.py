...
# class ModelValidator:  # Renamed from ModelTrainer to ModelValidator
#     def __init__(self, config: ModelValidationConfig) -> None:
#         self.config = config
#         self.X_val_transformed, self.y_val = self.load_and_prepare_data()

#     def load_and_prepare_data(self) -> Tuple[Any, pd.Series]:  # Corrected return type
#         """Loads and prepares the validation data."""
#         try:
#             data_manager = DataManager()
#             X_val_transformed, y_val = data_manager.load_validation_data(  #Corrected name as well
...
#     def validate(self, run_number: int):  # Renamed from train to validate
#         try:
...
#             return gb_model #Return a model but it doesnt change anything
#         except Exception as e:
...
#         model_validator = ModelValidator(config=model_validation_config)  # Renamed

#         # Validate the model, passing the run number
#         for i in range(1, 4):  # Example: Run validation 3 times
#             model = model_validator.validate(i) #Renamed
